...
export const ViolationsLogView = ({ setView }: ViolationsLogViewProps) => {
    ...
    // Derive violations from high/critical risk loans
    const violations = loans
        .filter(l => l.risk === 'High' || l.risk === 'Critical')
        .map(l => ({
            id: `V-${l.id.split('-').pop()}`,
            severity: l.risk,
            title: `High Risk Detected: ${l.counterparty}`,
            loan: `${l.id} (${l.counterparty})`,
            desc: l.reviewData?.summary || 'Automated risk detection flag.',
            deadline: l.deadline || 'Immediate',
            type: 'Risk'
        }));
                    ...
                    {violations.length > 0 ? (
                        violations.map((violation, i) => (
                            <div key={i} className="glass-panel p-6 rounded-2xl flex flex-col md:flex-row gap-6 items-start md:items-center group hover:border-red-500/30 transition-colors">
                                <div className={`p-4 rounded-xl shrink-0 ${violation.severity === 'Critical' ? 'bg-red-500/10 text-red-500' : 'bg-orange-500/10 text-orange-500'}`}>
                                    <ShieldAlert size={24} />
                                </div>
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-1">
                                        ...
                                        <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border ${violation.severity === 'Critical' ? 'text-red-400 border-red-500/30 bg-red-500/5' :
                                                'text-orange-400 border-orange-500/30 bg-orange-500/5'
                                            }`}>{violation.severity}</span>
                                    </div>
                                    <p className="text-sm text-text-muted mb-2 line-clamp-2">{violation.desc}</p>
                        ...
                        <div className="glass-panel p-10 text-center flex flex-col items-center">
                            ...
                            <p className="text-text-muted mt-2">No critical violations detected in your portfolio.</p>
                        </div>
                    )}
                </div>
            </div>
