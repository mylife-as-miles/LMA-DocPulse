...
const UploadView = ({ setView }: { setView: (v: ViewState) => void }) => (
      ...
      <div className="max-w-[1100px] w-full flex flex-col gap-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {/* Dropzone */}
            <div className="group relative flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-700 bg-surface-card hover:bg-surface-hover hover:border-brand-green transition-all duration-300 ease-out py-16 px-6 shadow-xl overflow-hidden border-glow">
              ...
              <div className="mt-8 z-10">
                <button className="px-8 py-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-white text-sm font-semibold transition-all backdrop-blur-sm group-hover:border-brand-green/50 group-hover:text-brand-green">
                    Select Files
                </button>
              </div>
            ...
            <div className="space-y-4">
              <h3 className="text-white text-lg font-display font-medium flex items-center gap-2">
                Upload Queue <span className="text-xs bg-surface-hover text-gray-400 px-2 py-0.5 rounded-full border border-border-dim">3</span>
              </h3>
              
              {/* Item 1: Progress */}
              <div className="group relative p-4 rounded-xl bg-surface-card border border-border-dim hover:border-gray-700 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="size-10 rounded-lg bg-surface-hover flex items-center justify-center text-brand-green border border-border-dim">
                    <FileText size={24} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between mb-1">
                      <p className="text-white text-sm font-medium truncate">LMA_Facility_Agreement_v2.pdf</p>
                      <span className="text-xs text-brand-green font-mono">45%</span>
                    </div>
                    <div className="w-full bg-surface-hover rounded-full h-1 overflow-hidden">
                      <div className="bg-brand-green h-full shadow-[0_0_10px_rgba(0,255,148,0.5)] relative" style={{ width: '45%' }}>
                        <div className="absolute inset-0 bg-white/30 w-full h-full animate-shimmer"></div>
                      </div>
                    </div>
                  </div>
                  <button className="size-8 flex items-center justify-center text-gray-500 hover:text-white transition-colors">
                    <X size={20} />
                  </button>
                </div>
              </div>
              ...
              {/* Item 2: Ready */}
              <div className="p-4 rounded-xl bg-surface-card border border-border-dim group">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="size-10 rounded-lg bg-surface-hover flex items-center justify-center text-blue-400 border border-border-dim">
                      <FileIcon size={24} />
                    </div>
                    <div className="min-w-0">
                      <p className="text-white text-sm font-medium truncate">Syndicated_Loan_Term_Sheet.docx</p>
                      <p className="text-gray-500 text-xs font-mono">1.8 MB • Ready</p>
                    </div>
                  </div>
              ...
              {/* Item 3: Error */}
              <div className="p-4 rounded-xl bg-surface-card border border-red-900/30 relative overflow-hidden">
                ...
                <div className="flex items-center justify-between pl-3 gap-4">
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="size-10 rounded-lg bg-surface-hover flex items-center justify-center text-red-400 border border-border-dim">
                      <ImageIcon size={24} />
                    </div>
                    <div className="min-w-0">
                      <p className="text-white text-sm font-medium truncate">scan_screenshot.jpg</p>
                      <p className="text-red-400 text-xs font-mono">Error: Format not supported</p>
                    </div>
                  </div>
            ...
            <div className="p-6 rounded-2xl bg-surface-card border border-border-dim shadow-xl flex flex-col gap-6 sticky top-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center p-3 rounded-lg bg-surface-hover border border-border-dim">
                  ...
                  <span className="text-white font-mono font-bold">~45s</span>
                </div>
              </div>
              <button onClick={() => setView('vault')} className="w-full group relative flex items-center justify-center gap-2 rounded-lg h-12 bg-brand-green hover:bg-[#33ffaa] text-pitch-black font-bold transition-all shadow-[0_0_20px_rgba(0,255,148,0.3)] hover:shadow-[0_0_30px_rgba(0,255,148,0.5)] overflow-hidden">
                <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300 skew-y-12"></div>
                <Bot size={20} className="z-10" />
                <span className="z-10">Analyze Documents</span>
              </button>
              <button className="w-full text-sm text-gray-500 hover:text-white transition-colors">Clear Queue</button>
            </div>
...
const DocumentVaultView = ({ setView }: { setView: (v: ViewState) => void }) => (
  <div className="flex-1 overflow-y-auto p-4 lg:p-8 pt-2 custom-scrollbar">
    <div className="mx-auto max-w-[1600px] flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div className="glass-panel rounded-2xl p-5 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-display font-bold text-white mt-1">1,248</h3>
          </div>
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-background text-text-main font-sans selection:bg-primary selection:text-black">
      ...
      <main className="flex flex-1 flex-col overflow-hidden relative bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#0a1e16] via-background to-background">
        ...
        {/* We hide the main app header in Upload view to match the immersive full-screen design provided, 
            or we can keep a minimal version. 
            The provided design has a header inside the content area. 
            Let's keep the Header for Dashboard/Vault, but maybe hide it for Upload for full immersion?
            Actually, let's keep it consistent for navigation purposes, but maybe change the title.
        */}
        ...
        {currentView === 'upload' && (
           /* For Upload View, we might need a mobile menu trigger if we hide the main header */
        ...
        {currentView === 'vault' && <DocumentVaultView setView={setCurrentView} />}
        {currentView === 'upload' && <UploadView setView={setCurrentView} />}
